...
public class Book {
	...
	public void Add (ulong positionKey, ushort moveValue) {
		if (!bookPositions.ContainsKey (positionKey)) {
			bookPositions.Add (positionKey, new BookPosition ());
		}

		bookPositions[positionKey].AddMove(moveValue);
	}

	public void Add (ulong positionKey, ushort moveValue, int numTimesPlayed) {
		if (!bookPositions.ContainsKey (positionKey)) {
			bookPositions.Add (positionKey, new BookPosition ());
		}

		bookPositions[positionKey].AddMove(moveValue, numTimesPlayed);
	}
}
...
public class BookPosition {
	...
	public void AddMove (ushort moveValue, int numTimesPlayed = 1) {
