...
public class AIPlayer
{
    ...
    public static void Init()
    {
        ...
        for (int depth = 1; depth < 64; depth++)
        {
            for (int count = 1; count < 64; count++)
            {
                ...
                Reductions[depth, count] = Mathf.Max(Mathf.RoundToInt(d * c / 2f) - 1, 0);
            }
        }
    ...
    {
        evaluationData = AI.Instance?.version1 ?? new();
    ...
    public static void FinishSearch()
    {
        ...
        if (AI.Instance != null)
        {
            AI.Instance._searchNodes = SearchNodes;
            AI.Instance._searchNodesPerDepth = SearchNodesPerDepth;
            AI.Instance._bestMovesThisSearch = BestMovesThisSearch;
            AI.Instance._depthReached = DepthReachedData;
            AI.Instance._nullMovePrunes = NullMovePrunes;
            AI.Instance._maxDepthReachedCount = MaxDepthReachedCount;
            AI.Instance._futilityPrunes =   FutilityPrunes;
            AI.Instance._moveCountBasedPrunes = MoveCountBasedPrunes;
        }
    }
    ...
    private static void StartSearch()
    {
        ...
        // BUG: Opening book no longer working.
        if (UseOpeningBook && AI.book.HasPosition(Board.ZobristKey))
        {
            MainLine.Move = AI.book.GetRandomBookMoveWeighted(Board.ZobristKey);
        ...
        else
        {
            ...
            while (!AbortSearch && (UseTimeLimit || depth <= TimeLimit /* Time limit also represents the max depth. */));
            SearchTime.Stop();
    ...
    public static int Search(int depth, int plyFromRoot, int alpha, int beta, int extensions, EvaluationData evaluationData, out Line pvLine, bool useNullMovePruning = true, int nullMoveSearchPlyFromRoot = 0, ulong previousCapture = 0, bool useMultiCut = true)
    {
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            if (depth > 2 && i > LateMoveThreshold[moves.Count] && !inCheck && !givesCheck)
            {
                ...
                {
                    LMR = true;
                    R += Reductions[Mathf.Min(depth, 63), Mathf.Min(i, 63)];
                }
            }
